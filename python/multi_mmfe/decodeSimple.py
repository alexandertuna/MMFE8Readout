...
def main(argv):
    ...
    decodedfile.write('FIFO\tBCIDtrig\tNtrig\tCH\tPDO\tTDO\tBCID\tVMM\tMMFE8\n')
#    num_trig = 0
...
#        numwordsread = numwordsread + linelength
        for iword in xrange(4, (len(thisline)), 2): #get rid of peak command/address and fifo bcid/num trig
            ...
            decodedfile.write(str(fifocount) + '\t' + str(bcid_trig) + '\t' + str(num_trig) + '\t' + '\t'+ str(addr) + '\t' + str(amp) + '\t' + str(timing) + '\t' + str(bcid_int) + '\t'+ str(vmm) +'\t'+ str(immfe)+'\n')
